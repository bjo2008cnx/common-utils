...
public class CN2SpellUtil {
    private static Map<String, Object> spellMap = null;
    ...
    public static int getCnAscii(char cn) {
        byte[] bytes;
        try {
            bytes = (String.valueOf(cn)).getBytes(GlobalConstant.CharSets.GBK);
        } catch (Exception e) {
    ...
    public static String getFirstSpell(String cnStr) {
        String fullSpell = getFullSpell(cnStr);
        if (fullSpell == null || fullSpell.length() == 0) {
            return null;
        } else {
            return fullSpell.substring(0, 1);
        }
    }
        ...
        for (int i = 0, Len = chars.length; i < Len; i++) {
            ...
            if (ascii == 0) { //取ascii时出错
                ...
                if (spell == null) {
                    ...
                    result.append(spell.substring(0, 1));
                }
            }
        }
    ...
    public static void main(String[] args) {
        String str = null;
        str = "谢海101普降喜雨";
        System.out.println("Spell=" + CN2SpellUtil.getFullSpell(str));
        System.out.println("Spell=" + CN2SpellUtil.getFirstSpell(str));
        System.out.println("Spell=" + CN2SpellUtil.getFirstSpells(str));
